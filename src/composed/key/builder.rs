...
mod tests {
    ...
    fn gen_x25519<R: Rng + CryptoRng>(mut rng: R) {
        let _ = pretty_env_logger::try_init();
    ...
    fn gen_ecdsa<R: Rng + CryptoRng>(mut rng: &mut R, curve: ECCCurve) {
        let _ = pretty_env_logger::try_init();
    ...
    fn gen_dsa<R: Rng + CryptoRng>(mut rng: &mut R, key_size: DsaKeySize) {
        let _ = pretty_env_logger::try_init();
