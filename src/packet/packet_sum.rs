...
use crate::{
    errors::Result,
    packet::{
        CompressedData, LibreOcbData, LiteralData, Marker, ModDetectionCode, OnePassSignature,
...
impl Serialize for Packet {
    fn to_writer<W: io::Write>(&self, writer: &mut W) -> Result<()> {
        match self {
            ...
            Self::LibreOcbData(p) => p.to_writer_with_header(writer),
        }
    }
    ...
    fn write_len(&self) -> usize {
        match self {
            ...
            Self::LibreOcbData(p) => p.write_len_with_header(),
        }
    }
}
...
impl PacketTrait for Packet {
    fn packet_header(&self) -> &PacketHeader {
        match self {
            ...
            Self::LibreOcbData(p) => p.packet_header(),
        }
    }
}
