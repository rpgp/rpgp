...
mod tests {
    ...
    fn test_psk() {
        ...
        let secret = SecretParams::Plain(PlainSecretParams::AEAD(aead_key::SecretKey {
            key: vec![0; SYM_ALG.key_size()],
        }));

        let psk = PersistentSymmetricKey::new(pk, secret).expect("foo");
        ...
        let SecretParams::Plain(PlainSecretParams::AEAD(ref aead)) = &psk.secret_params else {
            panic!("foo");
        };
