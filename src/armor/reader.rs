...
fn armor_header_type(i: &[u8]) -> IResult<&[u8], BlockType> {
    complete(alt((
        ...
        |i| {
            let (i, _) = tag("PGP MESSAGE, PART ")(i)?;
            let (i, x) = map_res(digit1, parse_digit)(i)?;
            let (i, y) = opt(preceded(tag("/"), map_res(digit1, parse_digit)))(i)?;
            Ok((i, { BlockType::MultiPartMessage(x, y.unwrap_or(0)) }))
        },
...
fn armor_header_line(i: &[u8]) -> IResult<&[u8], BlockType> {
    let (i, _) = armor_header_sep(i)?;
    let (i, _) = tag("BEGIN ")(i)?;
    let (i, typ) = armor_header_type(i)?;
    let (i, _) = armor_header_sep(i)?;
...
fn key_value_pair(i: &[u8]) -> IResult<&[u8], (&str, &str)> {
    let (i, key) = map_res(key_token, str::from_utf8)(i)?;
    let (i, value) = map_res(
        terminated(
...
fn footer_parser(i: &[u8]) -> IResult<&[u8], (Option<&[u8]>, BlockType)> {
    ...
    let (i, footer) = armor_footer_line(i)?;
    Ok((i, (crc, footer)))
}
