...
use crate::{
    ...
    ser::Serialize,
    types::MlKem768NistP384PublicParams,
};
...
/// Secret key for ML KEM 768 NIST P384
...
impl From<&SecretKey> for MlKem768NistP384PublicParams {
    fn from(value: &SecretKey) -> Self {
        Self {
            nistp384_key: value.nistp384.public_key(),
...
impl PartialEq for SecretKey {
    fn eq(&self, other: &Self) -> bool {
        self.nistp384.eq(&other.nistp384) && self.ml_kem_seed.eq(&other.ml_kem_seed)
    }
}
...
impl Serialize for SecretKey {
    ...
    fn write_len(&self) -> usize {
        NISTP384_KEY_LEN + ML_KEM768_KEY_LEN
    }
}
...
impl SecretKey {
    ...
    pub fn try_from_bytes(
        ...
        let secret = elliptic_curve::SecretKey::<p384::NistP384>::from_bytes(&nistp384.into())?;
...
pub struct EncryptionFields<'a> {
    /// Ephemeral Nist P 384 public key (97 bytes)
    pub ecdh_ciphertext: [u8; 97],

    pub ml_kem_ciphertext: &'a [u8; 1088],

    /// Recipient public key (97 bytes)
    pub ecdh_pub_key: &'a p384::PublicKey,
...
impl Decryptor for SecretKey {
    ...
    fn decrypt(&self, data: Self::EncryptionFields<'_>) -> Result<Zeroizing<Vec<u8>>> {
        debug!("ML KEM 768 NIST P384 decrypt");

        // Compute (ecdhKeyShare) := ECDH-KEM.Decaps(ecdhCipherText, ecdhSecretKey, ecdhPublicKey)
        let ecdh_key_share =
            ecdh_kem_decaps(&data.ecdh_ciphertext, &self.nistp384, data.ecdh_pub_key)?;
...
fn ecdh_kem_decaps(
    ...
    ecdh_secret_key: &p384::SecretKey,
    _ecdh_public_key: &p384::PublicKey,
) -> Result<[u8; 48]> {
    let ephemeral_public_key =
        elliptic_curve::PublicKey::<NistP384>::from_sec1_bytes(ecdh_ciphertext)?;
...
fn multi_key_combine(
    ...
    ecdh_key_share: &[u8; 48],
    ecdh_cipher_text: &[u8; 97],
    ecdh_public_key: &p384::PublicKey,
...
/// ML KEM 768 - NistP384 Encryption
///
...
pub fn encrypt<R: CryptoRng + Rng>(
    ...
    plain: &[u8],
) -> Result<([u8; 97], Box<[u8; 1088]>, Vec<u8>)> {
...
fn ecdh_kem_encaps<R: CryptoRng + Rng>(
    ...
    let our_secret = elliptic_curve::ecdh::EphemeralSecret::<NistP384>::random(&mut rng);

    let shared_secret = our_secret.diffie_hellman(public_key);

    let ephemeral_public = p384::PublicKey::from(&our_secret);
    let ephemeral_public = elliptic_curve::sec1::EncodedPoint::<NistP384>::from(ephemeral_public);
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        ...
        let pub_params: MlKem768NistP384PublicParams = (&skey).into();
        ...
        for text_size in (8..=248).step_by(8) {
            ...
            let data = EncryptionFields {
                ...
                ecdh_pub_key: &pub_params.nistp384_key,
