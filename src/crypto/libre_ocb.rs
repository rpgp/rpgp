...
impl<R: BufRead> StreamDecryptor<R> {
    pub fn new(
        ...
        let info = [
            Tag::LibreOcb.encode(), // packet type
            0x01,                   // version
            sym_alg.into(),
            aead.into(),
            chunk_size.into(),
    ...
    /// The nonce for OCB mode is computed by the exclusive-oring of the initialization vector
    /// as a 15-octet, big endian value, against the chunk index.
    fn nonce(iv: &[u8], chunk_index: &[u8; 8]) -> Vec<u8> {
        let mut nonce = iv.to_vec();
        let len = nonce.len();

        for (idx, value) in chunk_index.iter().rev().enumerate() {
            nonce[len - 1 - idx] ^= value;
        }
