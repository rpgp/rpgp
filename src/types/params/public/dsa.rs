...
impl DsaPublicParams {
    // Limits based on <https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf>, Section 4.2:
    //
    // > This Standard specifies the following choices for the pair L and N (the bit lengths of p
    // > and q, respectively):
    // > L = 1024, N = 160
    // > L = 2048, N = 224
    // > L = 2048, N = 256
    // > L = 3072, N = 256
    const MAX_L_BITS: usize = 3072;
    ...
    pub fn try_from_reader<B: BufRead>(mut i: B) -> Result<Self> {
        ...
        // The prime `p` is limited to L, `q` to N
            ...
            format!("p is too long ({} bytes)", p.len())
        );
        ensure!(
            q.len() * 8 <= Self::MAX_N_BITS,
            format!("q is too long ({} bytes)", q.len())
        );
        // `g` and `y` are both "mod p", so they are limited to the concrete length of `p`
        ensure!(
            g.len() <= p.len(),
            format!("g is longer than p ({} bytes)", g.len())
        );
        ensure!(
            y.len() <= p.len(),
            format!("y is longer than p ({} bytes)", y.len())
        );
